...
Unknown fields, incomplete files, placeholder hashes, non-approved review state, non-user-initiated distribution paths, parent traversal, and POSIX or Windows absolute paths fail validation. Both artifact paths and review-evidence paths are checked with platform-independent POSIX and Windows semantics. An absent `model-manifests/` directory passes while explicitly approving no model.
